...
export async function getDriverChecklist() {
  const { data } = await apiClient.get('/driver-shifts/checklist')
...
export async function getCurrentDriverShift() {
  const { data } = await apiClient.get('/driver-shifts/current')
...
export async function getDriverShiftHistory() {
  const { data } = await apiClient.get('/driver-shifts/history')
