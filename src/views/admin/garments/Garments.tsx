 ...
 * SK01
 */
function extractManualCode(fullCode: string, prefix?: string | null): string {
  if (!fullCode) {
    return ''
  }
  ...
  if (normalizedCode.startsWith(expectedStart)) {
    return fullCode.slice(expectedStart.length)
  }
 ...
 */
function buildPreviewCode(prefix?: string | null, manualCode?: string): string {
  const finalPrefix = prefix?.trim().toUpperCase() || ''
  const finalManualCode = normalizeManualCode(manualCode || '')
...
const Garments = () => {
  ...
  const previewCode = useMemo(
    () => buildPreviewCode(selectedClient?.code_prefix, form.code),
    [selectedClient, form.code],
  )
  ...
  const loadData = async () => {
    try {
      ...
      showBackendError(error, 'Error cargando prendas')
    } finally {
      setIsLoading(false)
    }
  }
  ...
  const handleChange = (field: keyof typeof emptyForm, value: string | boolean) => {
    ...
    if (field === 'code' && typeof value === 'string') {
      setForm((prev) => ({
        ...prev,
        code: normalizeManualCode(value),
      }))
  ...
  const handleSubmit = async () => {
    ...
    if (!form.code.trim()) {
      showAlert('El código de la prenda es obligatorio', 'warning')

      return
    }

    if (!selectedClient) {
      showAlert('Cliente no válido', 'warning')

      return
    }

    if (!selectedClient.code_prefix?.trim()) {
      showAlert('El cliente no tiene un prefijo configurado', 'warning')

      return
    }

    if (Number(form.value) < 0) {
      showAlert('El valor no puede ser negativo', 'warning')

      return
    }

    const confirmed = await confirmAction({
      title: editingId ? 'Actualizar prenda' : 'Crear prenda',
      ...
      fields: [
        ...
        {
          label: 'Valor',
          value: `$${Number(form.value || 0).toLocaleString('es-CL')}`,
        },
      ],
    })
    ...
    const payload = {
      ...
      code: normalizeManualCode(form.code),
    ...
    try {
      if (editingId) {
        await updateGarment(editingId, payload)

        showAlert('Prenda actualizada correctamente', 'success')
      } else {
        await createGarment(payload)

        showAlert('Prenda creada correctamente', 'success')
      }
      ...
      showBackendError(error, 'Error guardando prenda')
    }
  }
  ...
  const handleEdit = (garment: Garment) => {
    ...
    const client = clients.find((client) => client.id === garment.client_id)
     ...
     */
    const prefix = client?.code_prefix ?? garment.client?.code_prefix ?? ''
    ...
    setForm({
       ...
       * parte manual.
       */
      code: extractManualCode(garment.code, prefix),
  ...
  const handleDeactivate = async (id: string) => {
    ...
    const confirmed = await confirmAction({
      ...
      message: '¿Seguro que deseas desactivar esta prenda?',
      ...
      fields: [
        ...
        {
          label: 'Prenda',
          value: garment.description || garment.size || '-',
        },
      ],
    })
    ...
    try {
      ...
      showAlert('Prenda desactivada correctamente', 'success')

      await loadData()
    } catch (error) {
      showBackendError(error, 'Error desactivando prenda')
    }
  }
      ...
      <CCardBody>
        ...
        <CRow className="mb-3">
          ...
          <CCol md={2}>
            <CFormInput
              label="Prefijo"
              value={selectedClient?.code_prefix || ''}
              readOnly
              placeholder="-"
            />
          </CCol>
              ...
              <CTableRow key={garment.id}>
                ...
                <CTableDataCell>
                  <div className="d-flex gap-2">
                    <CButton color="warning" size="sm" onClick={() => handleEdit(garment)}>
                      Editar
                    </CButton>
