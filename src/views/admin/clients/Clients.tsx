...
const Clients = () => {
  ...
  const handleChange = (field: keyof typeof emptyForm, value: string) => {
    ...
    if (field === 'name') {
      setForm((prev) => ({
        ...prev,
        name: value,
        code_prefix: prefixManuallyEdited ? prev.code_prefix : generateClientPrefix(value),
      }))
  ...
  const handleSubmit = async () => {
    ...
    if (form.code_prefix.trim().length < 2) {
      showAlert('El prefijo debe tener al menos 2 caracteres', 'warning')
     ...
     */
    const duplicatePrefix = clients.find(
      (client) =>
        client.code_prefix?.toUpperCase() === form.code_prefix.trim().toUpperCase() &&
        client.id !== editingId,
    )
    ...
    const confirmed = await confirmAction({
      ...
      fields: [
        ...
        {
          label: 'Prefijo',
          value: form.code_prefix.toUpperCase(),
        },
        {
          label: 'RUT',
          value: form.rut,
    ...
    const payload = {
      ...form,
      name: form.name.trim(),
      code_prefix: normalizePrefix(form.code_prefix),
    }
      ...
      <CCardBody>
        ...
        <CRow className="mb-3">
          ...
          <CCol md={2}>
            ...
            <small className="text-body-secondary">Se usará en los códigos de prendas.</small>
          </CCol>
          ...
          <CCol md={6}>
            {form.code_prefix && (
              <div className="border rounded p-2">
                <small className="text-body-secondary">Ejemplo de código de prenda</small>

                <div>
                  <strong>
                    {normalizePrefix(form.code_prefix)}
                    -XXXX
                  </strong>
                </div>
              </div>
          ...
          <CTableBody>
            {clients.map((client) => (
              <CTableRow key={client.id}>
                ...
                <CTableDataCell>
                  <strong>{client.code_prefix || '-'}</strong>
                </CTableDataCell>
                ...
                <CTableDataCell>
                  <div className="d-flex gap-2">
                    <CButton color="warning" size="sm" onClick={() => handleEdit(client)}>
                      Editar
                    </CButton>
