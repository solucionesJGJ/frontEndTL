...
const Vehicles = () => {
  ...
  const loadVehicles = async () => {
    try {
      ...
      showBackendError(error, 'Error cargando vehículos')
    } finally {
      setLoading(false)
    }
  }
  ...
  const handleSubmit = async () => {
    if (!form.plate.trim()) {
      showAlert('La patente es obligatoria', 'warning')
    ...
    if (form.year !== '') {
      ...
      if (!Number.isInteger(year) || year < 1900 || year > 2100) {
        showAlert('Ingrese un año válido', 'warning')
    ...
    const duplicate = vehicles.find(
      (vehicle) =>
        normalizePlate(vehicle.plate) === normalizePlate(form.plate) && vehicle.id !== editingId,
    )

    if (duplicate) {
      showAlert(`Ya existe el vehículo ${duplicate.plate}`, 'warning')

      return
    }

    const confirmed = await confirmAction({
      title: editingId ? 'Actualizar vehículo' : 'Crear vehículo',
      message: editingId
      ...
      fields: [
        {
          label: 'Patente',
          value: normalizePlate(form.plate),
        },
        {
          label: 'Marca',
          value: form.brand || '-',
    ...
    try {
      ...
      showBackendError(error, 'Error guardando vehículo')
    } finally {
      setSaving(false)
    }
  }
  ...
  const handleDeactivate = async (vehicle: Vehicle) => {
    const confirmed = await confirmAction({
      ...
      message: 'El vehículo dejará de estar disponible para iniciar nuevas jornadas.',
    ...
    try {
      ...
      showAlert('Vehículo desactivado correctamente', 'success')
      ...
      showBackendError(error, 'Error desactivando vehículo')
    }
  }
  ...
  return (
    <>
      ...
      <CCard className="mb-4">
        <CCardHeader>
          <strong>{editingId ? 'Editar vehículo' : 'Nuevo vehículo'}</strong>
        </CCardHeader>
        ...
        <CCardBody>
          <CRow className="mb-3">
            ...
            <CCol md={1} className="d-flex align-items-end pb-2">
              <CFormCheck
                label="Activo"
                checked={form.active}
          ...
          {editingId && (
            <CAlert color="info">
              Estás editando un vehículo existente. Cambiar su información no modifica las jornadas
              históricas que ya fueron registradas.
            </CAlert>
          )}

          <div className="d-flex gap-2">
            <CButton color="primary" onClick={handleSubmit} disabled={saving}>
              {saving ? 'Guardando...' : editingId ? 'Actualizar vehículo' : 'Crear vehículo'}
            </CButton>

            {editingId && (
              <CButton color="secondary" onClick={handleCancel} disabled={saving}>
                Cancelar
              </CButton>
            )}
          </div>
      ...
      <CCard>
        <CCardHeader>
          <div className="d-flex justify-content-between align-items-center">
            ...
            <span className="text-body-secondary">{vehicles.length} registrado(s)</span>
          </div>
        </CCardHeader>
            ...
            <CTableBody>
              {vehicles.map((vehicle) => (
                <CTableRow key={vehicle.id}>
                  ...
                  <CTableDataCell>
                    <CBadge color={vehicle.active ? 'success' : 'secondary'}>
                  ...
                  <CTableDataCell>
                    <div className="d-flex gap-2">
                      <CButton color="warning" size="sm" onClick={() => handleEdit(vehicle)}>
                        Editar
                      </CButton>

                      {vehicle.active && (
                        <CButton color="danger" size="sm" onClick={() => handleDeactivate(vehicle)}>
                          Desactivar
                        </CButton>
                      )}
                    </div>
              ...
              {vehicles.length === 0 && (
                <CTableRow>
                  <CTableDataCell colSpan={6} className="text-center">
                    {loading ? 'Cargando vehículos...' : 'No hay vehículos registrados'}
                  </CTableDataCell>
                </CTableRow>
              )}
            </CTableBody>
